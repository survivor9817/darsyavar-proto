...
function showAnswer() {exerciseTabContainer.classList.toggle("open");}
...
function toggleFill(btn) {btn.classList.toggle("filled")}
function removeFill(btn) {btn.classList.remove("filled")}
function removeFeedback(feedbackElement) {feedbackElement?.classList.remove("tooltip-pop-in")}
function addFeedback(feedbackElement) {feedbackElement?.classList.add("tooltip-pop-in")}
...
inputBtnsContainer.addEventListener("click", (e) => {
    ...
    const correspondingTooltip = document.getElementById(clickedBtn.dataset.tooltipid);
    const previousTooltip = document.querySelector(".tooltips-list .tooltip-pop-in");
